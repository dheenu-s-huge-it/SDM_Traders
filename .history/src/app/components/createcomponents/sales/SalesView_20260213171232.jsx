    ...
    if (/^\d*\.?\d{0,2}$/.test(value)) {
      if (Number(value) >= 0) {
        ...
        const matchingToll = tollPriceData?.find(
          (item) =>
            sumamount >= item.from_amount && sumamount <= item.to_amount,
        );
        ...
        axiosGet
          ...
          .then((response) => {
            ...
            if (data.length !== 0) {
              ...
              setLuggageAmount(data[0]?.luggage);
              settotalAmount(data[0]?.total_amount);
              setNetAmount(data[0]?.net_amount);
            ...
            render={({ field }) => (
              <>
                ...
                <CustomTextField
                  ...
                  disabled
                  variant="outlined"
                  value={luggageAmount.toLocaleString("en-IN")}
