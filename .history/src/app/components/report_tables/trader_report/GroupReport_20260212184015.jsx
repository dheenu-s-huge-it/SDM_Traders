       ...
       <html>
         <head>
           ...
           <style>
             ...
             /* ✅ Replace with this - scoped only to items table */
.items-table {
  border-collapse: collapse; /* already present, keep it */
}

/* ✅ Add this to preserve outer borders on other tables */
section, table {
  box-sizing: border-box;
}
           </style>
  ...
  const grandTotal = groupedPrintData.map((traderWeekData) =>
    traderWeekData.trader_statement_data?.reduce(
      (sum, item) => sum + ((item.total_amount || 0) + (item.luggage || 0)),
      0
    )
  );
