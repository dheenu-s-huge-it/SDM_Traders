...
const GroupReport = () => {
  ...
  const grantTotalInWords = numberToWords(grandTotal);
          ...
          {groupedPrintData && groupedPrintData.length > 0 ? (
            groupedPrintData.map((traderWeekData, index) => (
              <div key={index} style={{ pageBreakInside: "avoid" }}>
                ...
                >
                  {groupedPrintData && groupedPrintData.length > 0 ? (
  groupedPrintData.map((traderWeekData, index) => {  // ✅ change => ( to => {

    // ✅ Define both variables here
    const grandTotal =
      traderWeekData.trader_statement_data?.reduce(
        (sum, item) =>
          sum + ((item.total_amount || 0) + (item.luggage || 0)),
        0
      ) || 0;
