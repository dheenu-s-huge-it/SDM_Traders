...
function EditFlowerTypeMaster({ Header, route_back }) {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const HandleEditGET = (value) => {
    if (value !== undefined && value !== null) {
      try {
        axiosGet
          .get(`master/flower_type/get?access_token=${token}&data_uniq_id=${value}`)
          .then((response) => {
            const data = response.data.data;
            if (data.length !== 0) {
                setflowerType(data[0]?.flower_type);
            }
          });
      } catch (error) {
        throw error;
  ...
  const handleSignIn = (e) => {
    ...
    try {
      axiosPost.put("master/flower_type/create", jsonStructure)
        .then((response) => {
...
export default EditFlowerTypeMaster;
