...
function EditIncomeTypeMaster({ Header, route_back }) {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const HandleEditGET = (value) => {
    if (value !== undefined && value !== null) {
      try {
        axiosGet
          .get(`master/income/get?access_token=${token}&data_uniq_id=${value}`)
          .then((response) => {
            const data = response.data.data;
            if (data.length !== 0) {
                setIncomeType(data[0]?.income_type);
            }
          });
      } catch (error) {
        throw error;
  ...
  const handleSignIn = (e) => {
    ...
    try {
      axiosPost.put("master/income/create", jsonStructure)
        .then((response) => {
              ...
              render={({ field }) => (
                <>
                  ...
                  <CustomTextField
                    ...
                    error={!!errorsMessage.income_type}
                    helperText={
                      errorsMessage.income_type ? errorsMessage.income_type : ""
                    }
                    sx={{ mb: 5 }}
                  />
                </>
...
export default EditIncomeTypeMaster;
