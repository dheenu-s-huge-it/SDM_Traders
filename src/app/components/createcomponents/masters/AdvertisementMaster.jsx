...
function AdvertisementMaster({ Header, route_back }) {
  const {
    control,
    handleSubmit,
    formState: { errors },
...
const handleRemoveImage = () => {
  ...
  const handleSignIn = (e) => {
    ...
    const jsonStructure = {
        access_token: token,
        remarks: remarks,
        image_name: base64Image?.image_name,
        advertisement_master_doc:base64Image?.document_doc,
    };
    try {
      axiosPost
        .post("master/advertisement/create", jsonStructure)
        .then((response) => {
  ...
  return (
    <Card sx={{ padding: 10 }}>
      ...
      <Snackbar
        open={errorImg.message !== ""}
        anchorOrigin={{
          vertical: "top",
          horizontal: "right",
        }}
        message={errorImg.message}
        onClose={() => setErrorImg({ status: "", message: "" })}
        autoHideDuration={2500}
      >
        <Alert onClose={handleClose} severity={errorImg.status}>
          {errorImg.message}
        </Alert>
      </Snackbar>
    </Card>
