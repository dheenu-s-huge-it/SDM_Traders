...
function EditDocumentMaster({ Header, route_back }) {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const HandleEditGET = (value) => {
    if (value !== undefined && value !== null) {
      try {
        axiosGet
          .get(`master/document/get?access_token=${token}&data_uniq_id=${value}`)
          .then((response) => {
            const data = response.data.data;
            if (data.length !== 0) {
              setDocumentTypeName(data[0]?.document_type);
            }
          });
      } catch (error) {
        throw error;
  ...
  const handleSignIn = (e) => {
    ...
    try {
      axiosPost.put("master/document/create", jsonStructure)
        .then((response) => {
        ...
        <Grid container spacing={6}>
          <Grid item xs={12} sm={4}>
            <Controller
              ...
              render={({ field }) => (
                <>
                  {renderLabel("Document Name", true)}
                  <CustomTextField
                    {...field}
                    fullWidth
                    variant="outlined"
                    ...
                    onChange={(e) => {
                      ...
                      if (errorsMessage.document_type) {
                    ...
                    error={!!errorsMessage.document_type}
                    helperText={
                      errorsMessage.document_type ? errorsMessage.document_type : ""
                    }
                    sx={{ mb: 5 }}
                  />
                </>
...
export default EditDocumentMaster;
