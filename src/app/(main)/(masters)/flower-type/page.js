...
const AllTraders = () => {
  useEffect(() => {
    document.title = "SDM FLOWER TYPE";
  }, []);
  ...
  const HandleRouteCreate = () => {
    router.push("/flower-type/create");
  };
  ...
  const fetchData = async (field = orderField, type = orderType) => {
    ...
    axiosGet
      .get(
        `master/flower_type/get?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchFinal}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${type}&order_field=${field}`
      )
      .then((response) => {
  ...
  const fetchDataFilter = async () => {
    ...
    axiosGet
      .get(
        `master/flower_type/get?access_token=${ACCESS_TOKEN}&items_per_page=10000`
      )
      .then((response) => {
  ...
  const handleStatusChange = () => {
    ...
    axiosPost
      .post(`master/flower_type/status`, jsonData)
      .then((response) => {
