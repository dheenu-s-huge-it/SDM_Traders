    ...
    root: ({ level }) => ({
      ...(!isPopoutWhenCollapsed || popoutExpanded || (popoutCollapsed && level === 0)
        ...
        : {
          ...
          [`& .${menuClasses.button}`]: {
            padding: theme.spacing(2) // Reduce left and right padding for the button
          },
        
          // Keep the padding of the submenu content unaffected
          // [`& .${menuClasses.subMenuContent}`]: {
          //   padding: theme.spacing(1) // Ensure submenu content retains its original padding
          // },
        ...
        ...(popoutCollapsed && level > 0
          ? {
              ...
              [`& .${menuClasses.icon}`]: {
                ...
                color: '#FFFFFF' // Set label color to white when active
              },
